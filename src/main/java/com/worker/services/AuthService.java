...
public class AuthService extends JSONService {
	...
	public static final String ISSUER = "workerapp.com";
	...
	public Response authenticateCredentials(@HeaderParam("email") String email,
		...
		Usuario user = null;
		try {
			user = UsuarioEM.getInstance().getUsuario(email, password);
			System.out.println(String.format("AuthService: email=%s, pwd=%s, user=%s", email, password, user));
		...
		claims.setNotBeforeMinutesInThePast(2); // time before which the token
												...
												// ago)
		claims.setSubject( "" + user.getId() ); // the subject/principal is whom
											// the token is about
